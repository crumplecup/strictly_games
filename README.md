...
| Game | ratatui TUI | egui (native) | Leptos (HTTP) | MCP Agent | Kani Proofs | Verified IR |
| --- | :---: | :---: | :---: | :---: | :---: | :---: |
| Tic-Tac-Toe | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| Blackjack | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| Craps | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |

All three games render through a WCAG-verified [AccessKit](https://accesskit.dev) IR
(`VerifiedTree`) that feeds **three parallel frontends** — ratatui terminal, egui native
window, and leptos HTTP server — identically, with the same proof tokens flowing through
each path.
...
### 5. Verified UI (IR-to-Frontend Contracts)

Every game state is first converted to a **WCAG-verified AccessKit IR** (`VerifiedTree`),
then rendered to three parallel frontends.  The proof chain is frontend-agnostic:

```rust
// Step 1 — build the IR (same for all three frontends)
let tree: VerifiedTree = ttt_to_verified_tree(&game, &mode, &log, &graph, viewport);
//                        ↑ WcagVerified anchor lives here

// Step 2 — render (backend-specific, but same proof shape)
// ratatui:
let (widget, _stats, render_proof) = RatatuiBackend::new().render(&tree)?;
// egui:
let (widget, _stats, render_proof) = EguiBackend::new().render(&tree)?;
// leptos:
let (html, _stats, render_proof) = LeptosRenderer::html().render(&tree)?;

// Step 3 — gate on the frontend-agnostic game consistency proof
let _proof: Established<TttUiConsistent> = Established::prove(&render_proof);
```

`TttUiConsistent` implements `ProvableFrom<Established<RenderComplete>>` — the compiler
enforces that a complete IR-to-frontend render happened before you can name the token.

**Kani verifies the IR arithmetic** behind layout props across seven terminal breakpoints:
...
### Quick Start: Terminal TUI (ratatui)
...
The TUI shows the full architecture at work:
...
**Controls:** Arrow keys to navigate, Enter to confirm, `q` to quit.
...
Opens a native window rendered with egui via winit+wgpu.  Uses the same AccessKit IR as
the TUI — the proof chain is identical.

**TTT keyboard controls inside the egui window:**
...
### HTTP Server (leptos + axum)

```bash
cargo run http --port 3000
```

Serves WCAG-compliant HTML generated by the leptos renderer from the same `VerifiedTree`
IR.  Visit `http://localhost:3000/games/ttt`, `/games/blackjack`, `/games/craps`.

Alternatively, embed the router programmatically:

```rust
use strictly_server::{run_leptos, run_egui, LeptosAppState, leptos_game_router};

// Blocking native window:
run_egui();

// Async HTTP server:
run_leptos(3000).await?;
```
...
All game states flow through a shared WCAG-verified AccessKit IR before reaching any frontend.

```text
GameState (TTT / Blackjack / Craps)
  └→ *_to_verified_tree()       ← WcagVerified credential anchored here
       └→ VerifiedTree (IR)
            ├── RatatuiBackend::render()  → TuiNode → render_node() → Terminal
            ├── EguiBackend::render()     → egui widgets → wgpu / winit window
            └── LeptosRenderer::html()   → HTML + CSS → axum HTTP response
```

**Proof chain** (identical for all three frontends):
`WcagVerified → Established<RenderComplete> → Established<*UiConsistent>`
...
│   ├── strictly_server/      # MCP server + all three frontends
│   │   └── src/
│   │       ├── egui_frontend.rs    # Native window: winit + wgpu + egui
│   │       ├── leptos_frontend.rs  # HTTP server: axum + leptos renderer
│   │       └── tui/
│   │           ├── game_ir.rs      # *_to_verified_tree() IR builders (shared)
│   │           ├── contracts.rs    # *UiConsistent props + ProvableFrom impls
│   │           ├── palette.rs      # WCAG-verified GamePalette
│   │           ├── blackjack.rs    # Ratatui render path, proof-token-flow
│   │           ├── craps.rs        # Ratatui render path, proof-token-flow
...
- **[elicitation](https://github.com/crumplecup/elicitation)** — Type-safe elicitation, MCP integration, contract system, TUI/egui/leptos rendering
- **rmcp** — Model Context Protocol implementation
- **ratatui** — Terminal UI
- **egui + egui-wgpu + egui-winit** — Native GUI
- **wgpu + winit** — GPU rendering and windowing
- **axum + leptos** — HTTP server and HTML renderer
- **tokio** — Async runtime
...
// every render path must produce a Established<RenderComplete>
// before the game-consistency token can be named
impl ProvableFrom<Established<RenderComplete>> for TttUiConsistent {
    fn prove_from(from: &Established<RenderComplete>) -> Established<TttUiConsistent> {
        // Compiler enforces: you cannot call this without a render proof
        Established::prove(from)
    }
}
```
