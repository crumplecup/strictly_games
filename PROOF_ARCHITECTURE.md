...
crates/strictly_verus/src/generated/   # AUTO-GENERATED by strictly_verus/build.rs
    ├── blackjack.rs   ← self-contained VSM contracts (inline stubs + assume_specification)
    ├── craps.rs
    └── tictactoe.rs
```

The `verus_proofs/generated/` directory is produced by `cargo build -p strictly_proofs`.
The `strictly_verus/src/generated/` directory is produced by `cargo build -p strictly_verus`.
Both `verify-verus-tracked` and `verify-verus-resume` run the `strictly_proofs` build
automatically before iterating.  `verify-verus-vsm` runs the `strictly_verus` build.
...
the body.  These contracts are generated by `strictly_verus/build.rs` using:
