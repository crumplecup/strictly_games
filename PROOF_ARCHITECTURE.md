...
crates/strictly_verus/src/generated/   # AUTO-GENERATED by `elicitation generate verus`
    ├── blackjack.rs   ← self-contained VSM contracts (inline stubs + assume_specification)
    ├── craps.rs
    └── tic_tac_toe.rs
```

The `verus_proofs/generated/` directory is produced by the generate flow for
`strictly_proofs`.
The `strictly_verus/src/generated/` directory is produced by
`elicitation generate verus`.
Both `verify-verus-tracked` and `verify-verus-resume` run the `strictly_proofs`
generation/build flow automatically before iterating. `verify-verus-vsm` should
target the generated files in `strictly_verus/src/generated/`.

---

## VSM Verification — `strictly_verus`

The VSM transitions (`bj_start_betting`, `craps_come_out`, etc.) are `async fn`s
that use tokio and database drivers.  Verus cannot verify them from source.

The solution is **`assume_specification` contracts**: a trusted axiom that tells
Verus "this function preserves the invariant" without requiring Verus to verify
the body. These contracts are generated by `elicitation generate verus` using:

```
CrapsMachine::vsm_verus_transitions()   →   proc_macro2::TokenStream
                │ .to_string()
                ▼
crates/strictly_verus/src/generated/craps.rs
                │
                ▼
verus --crate-type=lib craps.rs    ✅  8 verified, 0 errors
```

Each generated file is **fully self-contained** — no external crate imports.
It defines stub types inline, then axiomatises each transition via
`assume_specification`.

### `vsm_verus_transitions()` vs `vsm_verus_proof()`

| Method | Source | What it generates |
|--------|--------|-------------------|
| `vsm_verus_proof()` | `VerifiedStateMachine` trait | `assume_specification` using the live state types from the game crate (requires crate imports, used in `strictly_proofs/generated/*_vsm.rs`) |
| `vsm_verus_transitions()` | `VerifiedStateMachine` trait | `#[verifier::external]` stubs + `assume_specification` (fully standalone; used in `strictly_verus/src/generated/*.rs`) |

`verify-verus-vsm` proves the VSM invariant with the self-contained approach.
`verify-verus-tracked` verifies business-logic Verus proof files (foundation,
type invariants, etc.) that don't need VSM contracts.
...
just verify-kani-vsm              # VSM-only Kani harnesses; CSV tracking
just verify-kani-vsm-resume       # skip already-PASS harnesses
...
just verify-verus-vsm             # VSM-only Verus contracts; CSV tracking
just verify-verus-vsm-resume      # skip already-PASS files
