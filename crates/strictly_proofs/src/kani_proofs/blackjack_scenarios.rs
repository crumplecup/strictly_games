...
//! shoes built with [`Shoe::from_ordered`].  Unlike the unit-level harnesses in
...
//!
//! - **Trust**: Rust ownership, `Shoe::from_ordered` determinism, `Established::assert()`
//! - **Verify**: Workflow integration, outcome correctness, financial conservation

#[cfg(kani)]
use strictly_blackjack::{
    BasicAction, BetPlaced, Card, GameBetting, GameResult, Outcome, PayoutSettled, PlaceBetOutput,
    PlayActionOutput, PlayActionResult, PlayerTurnComplete, Rank, Shoe, Suit, execute_dealer_turn,
    execute_place_bet, execute_play_action,
};

// ── Helpers ───────────────────────────────────────────────────────────────────

/// Build a `GameBetting` with a fully deterministic ordered shoe.
///
/// Cards are laid out for 2-player, 2-dealer deal pattern:
/// `generate()` returns cards in order: p1, d1, p2, d2, then remaining...
///
/// So `[c0, c1, c2, c3, ...]` → player gets `[c0, c2]`, dealer gets `[c1, c3]`.
#[cfg(kani)]
fn betting_with_shoe(cards: &[Card]) -> GameBetting {
    GameBetting::new(Shoe::from_ordered(cards), 1000)
}
...
///
/// Shoe layout: player gets [Seven♠, Eight♥] = 15, dealer gets [Ace♣, King♣] = blackjack.
///
...
///
/// Shoe layout: player gets [Ace♠, King♠], dealer gets [Ace♣, Queen♣].
///
...
///
/// Shoe: player [Nine♠, Nine♥]=18, dealer [Six♣, Ten♦]=16.
/// Dealer stands at 16 (fixed rule: stand on 17+, but here dealer has 16 and
/// no more hits needed since shoe is exhausted after initial deal).
...
///
/// Shoe: player [King♠, Queen♥]=20, then player hits Ten♣ → 30 (bust).
...
fn scenario_bankroll_conservation() {
    // Concrete shoe: player 20 (King+King), dealer 16 (Six+Ten) → hits Two → 18.
    ...
    // betting_with_shoe uses bankroll=1000
    ...
    let (pt, bet_proof) = match place_output {
        PlaceBetOutput::PlayerTurn(pt, proof) => (pt, proof),
        PlaceBetOutput::Finished(..) => panic!("no natural in this shoe"),
    };
