...
//!
//! These proofs cover the types and functions in `strictly_tictactoe` and form
//! a three-tier compositional verification chain.
//!
//! ## Proof composition (Kani function contracts)
//!
//! **Tier 1 — leaf contracts** (`#[kani::proof_for_contract]`):
//! Each sub-function has a formal contract (`#[kani::ensures]` / `#[kani::requires]`
//! on the source) and a dedicated harness that proves that contract holds for all
//! possible inputs.  These are the leaves of the proof tree.
//!
//! **Tier 2 — compositional capstone** (`#[kani::stub_verified]`):
//! `make_move_alternates_player_compositional` proves the alternation property by
//! replacing `validate_move` and `execute_move` with their *verified contracts*
//! (stubs).  It never re-explores the sub-function bodies; it trusts the proven
//! contracts as axioms.  This is genuine proof composition: the `make_move` proof
//! depends on and explicitly references the proofs for its sub-functions.
//!
//! **Tier 0 — structural foundation** (`generated/tictactoe_foundation.rs`):
//! Auto-generated by `build.rs` via `Type::kani_proof()` — proves constructibility
//! and `Arbitrary` for all TicTacToe types.  These underpin every harness above.
//!
//! ## What is verified
//!
...
fn validate_square_empty_err_when_occupied() {
    ...
    let Ok(GameResult::InProgress(game)) = result else {
        return;
    };
...
// ─────────────────────────────────────────────────────────────
//  Tier-1 contract proofs (proof_for_contract)
//
//  Each harness proves that the source function's #[kani::ensures] /
//  #[kani::requires] contract holds for all possible inputs.  Kani
//  automatically injects the requires as assumptions and asserts the
//  ensures after the call.
// ─────────────────────────────────────────────────────────────

/// Proves the contract for `validate_square_empty`:
/// `result.is_ok() ↔ board.is_empty(position)` — for all inputs.
#[cfg(kani)]
#[kani::proof_for_contract(validate_square_empty)]
fn contract_validate_square_empty() {
    let game: GameInProgress = kani::any();
    let mov: Move = kani::any();
    validate_square_empty(&mov, &game);
}

/// Proves the contract for `validate_player_turn`:
/// `result.is_ok() ↔ mov.player == game.to_move()` — for all inputs.
#[cfg(kani)]
#[kani::proof_for_contract(validate_player_turn)]
fn contract_validate_player_turn() {
    let game: GameInProgress = kani::any();
    let mov: Move = kani::any();
    validate_player_turn(&mov, &game);
}

/// Proves the contract for `validate_move`:
/// `result.is_ok() ↔ (board.is_empty(pos) ∧ player == to_move)` — for all inputs.
///
/// This is the AND-composition of the two sub-contracts, mirroring `LegalMove =
/// And<SquareEmpty, PlayerTurn>` at the type level.
#[cfg(kani)]
#[kani::proof_for_contract(validate_move)]
fn contract_validate_move() {
    let game: GameInProgress = kani::any();
    let mov: Move = kani::any();
    validate_move(&mov, &game);
}

/// Proves the contract for `execute_move`:
/// Given the requires hold (empty square, correct player), the square becomes
/// `Occupied(player)`, `to_move` is preserved, and history grows by one.
///
/// The `kani::assume` calls satisfy the requires so that `validate_move`
/// returns `Ok` (providing the necessary proof token).  Kani also injects
/// the requires automatically before the contracted call.
#[cfg(kani)]
#[kani::proof_for_contract(execute_move)]
fn contract_execute_move() {
    let mut game: GameInProgress = kani::any();
    let mov: Move = kani::any();
    // Assume the requires so validate_move succeeds and we can obtain the proof token.
    kani::assume(game.board().is_empty(mov.position));
    kani::assume(mov.player == game.to_move());
    let proof = validate_move(&mov, &game).unwrap();
    execute_move(&mov, &mut game, proof);
}
...
/// **Compositional proof** — `make_move` alternates `to_move` using verified sub-contracts.
///
/// Uses `#[kani::stub_verified]` to replace `validate_move` and `execute_move`
/// with their formally proven contracts (see `contract_validate_move` and
/// `contract_execute_move`).  This harness never re-explores the sub-function
/// bodies — it trusts the proven contracts as axioms and only reasons about
/// the coordination logic in `make_move` itself.
///
/// Composition chain:
/// ```text
/// contract_validate_square_empty  ─┐
///                                   ├─► contract_validate_move ─┐
/// contract_validate_player_turn  ─┘                             │
///                                                                ├─► make_move_alternates_player_compositional
/// contract_execute_move  ────────────────────────────────────────┘
/// ```
#[cfg(kani)]
#[kani::proof]
#[kani::stub_verified(validate_move)]
#[kani::stub_verified(execute_move)]
fn make_move_alternates_player_compositional() {
    let pos: Position = kani::any();
    let game = GameSetup::new().start(Player::X);
    let initial_to_move = game.to_move(); // always X on fresh game
    let action = Move::new(initial_to_move, pos);

    match game.make_move(action) {
        Ok(GameResult::InProgress(g)) => {
            assert_eq!(
                g.to_move(),
                initial_to_move.opponent(),
                "to_move must flip to opponent after a valid non-terminal move",
            );
        }
        // With a nondeterministic execute_move stub the board may appear full or
        // winning — we focus the compositional claim on the InProgress outcome.
        Ok(GameResult::Finished(_)) | Err(_) => {}
    }
}
...
fn replay_two_moves_alternates_and_records() {
    ...
    let result = GameInProgress::replay(&[Move::new(Player::X, pos1), Move::new(Player::O, pos2)])
